...
export interface SolvePowBrowserOptions {
  // 并发 worker 数量，默认 max(1, hardwareConcurrency - 1)
  workers?: number;
  // nonce 起始值，默认 0
  startNonce?: number;
  // 总尝试次数上限（所有 worker 合计）
...
function mustNumber(raw: Record<string, unknown>, key: string): number {
  const v = raw[key];
...
export function decodePowChallenge(powToken: string): PowChallenge {
  ...
  const raw = JSON.parse(TEXT_DECODER.decode(payloadBytes)) as Record<
    string,
    unknown
  >;
  ...
  return {
    v: mustNumber(raw, "v"),
    action: mustString(raw, "action"),
    target_type: mustString(raw, "target_type"),
    target_id: mustNumber(raw, "target_id"),
    user_id: mustNumber(raw, "user_id"),
    issued_at: mustNumber(raw, "issued_at"),
    expires_at: mustNumber(raw, "expires_at"),
    difficulty: mustNumber(raw, "difficulty"),
    salt: mustString(raw, "salt"),
  };
}

function buildPowPrefix(challenge: PowChallenge): Uint8Array {
  return concatBytes([
...
export async function solvePowNonceBrowser(
  powToken: string,
  options: SolvePowBrowserOptions = {}
): Promise<string> {
